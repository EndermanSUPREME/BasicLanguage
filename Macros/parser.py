...
def parse_factor(tokens):
    ...
    if token["tag"] == "boolean": # within parse_factor
...
def parse_nallahab_statement(tokens):
    """
    nallahab_statement = "nallahab";
    """
    assert tokens[0]["tag"] == "kentid", f"Expected 'kentid', got {tokens[0]}"
    tokens = tokens[1:]
...
def parse_statement(tokens):
    ...
    if tag == "kentid": # within parse_statement
...
"""
    ...
    (factor) MACRO = "_" identifier "_"
    MACRO_CALL = MACRO ;
    MACRO_DEF =  MACRO "=" identifier_list statement_block

    Macros are stored within a Macros_list where during evaluation they can be looked up when called.
    During Parsing we can introduce new Macros to this list.
"""
def test_parse_macro():
    print("testing parse_macro()")
    for s in ["_LINE_", "_ASPHALT_", "_MAC_"]:
        tokens = tokenize(s)
        ast, tokens = parse_factor(tokens)
...
def parse_macro_definition(tokens):
    ...
    """
    macro = tokens[0]["value"]
    assert tokens[0]["tag"] == "macro_assign", f"Expected 'macro_assign', got {tokens[0]}"
    tokens = tokens[1:]

    # print("Parsing Identifier List. . .")
    parameters, tokens = parse_identifier_list(tokens)
    # print("Parsing Statement Block. . .")
    statements, tokens = parse_statement_block(tokens)
...
def test_parse_macro_definition():
    ...
    ast, tokens = parse_macro_definition(tokens)

    assert ast == {
        "tag": "macro_definition",
        "value": {
            "macro_target": "__ENDER__=",
            "parameters" : {"tag": "identifier_list", "identifiers": []},
            "statements" : {"tag": "block", "statements": []}
        }
    }, f"{ERR} Malformed Macro Definition AST!, Got --> {ast}"
    assert tokens[0]["tag"] == None, f"{ERR} Expected End of Token Stream, got {tokens[0]['tag']}"
...
if __name__ == "__main__":
    ...
    print("done.")
    """
    print(f"{INFO} Showing Off Final Project!")
    test_parse_macro()
    test_parse_macro_call()
    test_parse_macro_definition()
    print(f"{OK} DONE!")