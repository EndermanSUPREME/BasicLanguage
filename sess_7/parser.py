...
def test_parse_stmt():
    ...
    try:
        # testing for bad identifiers
        tokens = tokenizer.tokenize("steal 3*9")
        ast, tokens = parse_stmt(tokens)
        print(f"{tokenizer.ERR} Unrecognized Identifier not flagged")
        exit(1)
    except Exception as e:
        print(f"{tokenizer.OK} Successfully Detected Unrecognized Identifier!")
        print(e)
...
def parse_stmt(tokens):
    ...
    if tokens[0]["tag"] == "identifier":
        ...
        assert tokens[0]["value"] in tokenizer.accepted_identifiers, f"{tokenizer.ERR} Syntax Error, no known identifier -> {tokens[0]['value']}"
