...
def parse_stmt(tokens):
    ...
    """
    statement = statement_block | if_stmt | while_stmt | function_stmt | return_stmt | print_stmt | kentID_stmt | assignment_stmt ;
    """
    tag = tokens[0]["tag"]
    if tag == "l_curly":
    ...
    if tag == "kentID":
        return parse_kentid_stmt(tokens)
    if tag == "macro":
        return parse_macro_stmt(tokens)
...
def parse_macro_literal(tokens):
    """
    MACRO_LITERAL = macro
    """
    target = tokens[0]
    assert target["tag"] == "macro", f"{tokenizer.ERR} Expected the tag: macro, recieved -> {target['tag']}"
    tokens = tokens[1:]
...
def test_parse_macro_literal():
    print(f"{tokenizer.INFO} Testing Parse Macro Statement. . .")
    tokens = tokenizer.tokenize("__MyMacro__ = 12")
    ast, tokens = parse_macro_stmt(tokens)
    print(ast)
    assert ast == {
        "tag": "assign",
        "target": {"tag": "macro", "value": "__MyMacro__"},
        "value": {"tag": "number", "value": 12},
    }
    print(f"{tokenizer.OK} Parse Marco Statement Rule Test Passed!")

def parse_macro_stmt(tokens):
    """
    MACRO_STMT = MACRO_LITERAL + "=" { literal | expression }
    """
    assert tokens[0]["tag"] == "macro", f"{tokenizer.ERR} Expected the tag: macro, recieved -> {tokens[0]['tag']}"

    # consumes the macro token
    target, tokens = parse_macro_literal(tokens)
...
def test_parse_macro_stmt():
    ...
    assert ast == {
        ...
        "target": {"tag": "macro", "value": "__MyMacro__"},
        "value": {"tag": "number", "value": 12},
    }, f"{tokenizer.ERR} Did not parse macro statement correctly!"
    print(f"{tokenizer.OK} Parse Marco Statement Rule Test Passed!")


def parse_kentid_stmt(tokens):
    """
    kentid_stmt = "nallahab" ;
    """
    assert tokens[0]["tag"] == "kentID"
    tokens = tokens[1:]
    if tokens[0]["tag"] in [";", None]:
        # value will be later pushed to the env variable __kentid__
        return {"tag": "kentID", "value": 'nallahab@kent.edu'}, tokens
    else:
        print(f"{tokenizer.ERR} Unexpected tag: {tokens[0]['tag']}, Expected ';' or 'None'")
        exit(1)
def test_parse_kentid_stmt():
    """
    kentid_stmt = "nallahab" ;
    """
    print(f"{tokenizer.INFO} Testing Parse KentID Statement. . .")
    ast = parse_kentid_stmt(tokenizer.tokenize("nallahab;"))[0]
    assert ast == {"tag": "kentID", "value": "nallahab@kent.edu"}
    print(f"{tokenizer.INFO} KENT-ID AST: {ast}")
    print(f"{tokenizer.OK} Testing Parse KentID Statement Passed!")
...
def main():
    ...
    test_parse_macro_stmt()

    test_parse_function_literal()
    print(f"{tokenizer.INFO} Test Parse Function Statement In Development. . .")

    print(f"{tokenizer.INFO} Running KentID statement test. . .")
    test_parse_kentid_stmt()
